...
exports.get_dt = (payload) => {
    const dates = []
    const extract = (obj, key) => {
        if (obj && typeof obj === 'object') {
            if (obj[key]) dates.push(new Date(obj[key]))
            Object.values(obj).forEach(val => extract(val, key))
        }
    }
    
    extract(payload, 'observedAt')
    if (!dates.length) extract(payload, 'modifiedAt')
    if (!dates.length) dates.push(new Date())
    
    const latest = new Date(Math.max(...dates))
    return latest.toISOString().slice(0, 13).replace('T', '-')
}
