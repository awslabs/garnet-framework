    ...
    try {
        ...
        if(['CREATED', 'UPDATED'].includes(operation)){
            ...
            if(parentGroupName){
                ...
                group.scope = '/' + parentGroupName + `/` + thingGroupName
            }
            ...
            if(rootToParentThingGroups && rootToParentThingGroups.length > 0 ){
                ...
                group.scope = '/' + rootToParentThingGroups.map(tg => tg.groupArn.split('/').pop()).join('/') + `/` + thingGroupName
            }
        ...
        if(['DELETED'].includes(operation)){
            const headers = await broker_headers()
            let {data: res} = await axios.delete(`${dns_broker}/entities/urn:ngsi-ld:${AWSIOTTHINGGROUPTYPE}:${thingGroupName}`, {headers})
            console.log(res)
        }
