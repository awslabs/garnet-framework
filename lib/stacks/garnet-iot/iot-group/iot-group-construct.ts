    ...
    constructor(scope: Construct, id: string, props: GarnetIotGroupProps) {
      super(scope, id)
        ...
        */
        ...
        // LAMBDA TO UPDATE BROKER WITH THING GROUP HIERARCHY
        const lambda_update_group_hierarchy_log = new LogGroup(this, 'GarnetIoTGroupHierarchyLambdaLogs', {
          retention: RetentionDays.ONE_MONTH,
          removalPolicy: RemovalPolicy.DESTROY
        })
        const lambda_update_group_hierarchy_path = `${__dirname}/lambda/groupHierarchy`;
        const lambda_update_group_hierarchy = new Function(this, "GarnetIoTGroupHierarchyLambda", {
          functionName: `${garnet_nomenclature.garnet_iot_group_hierarchy_lambda}`,
          description: 'Garnet Sync AWS IoT Things Group Hierachy- Function that updates Things Group Hierachy for Things',
          vpc: props.vpc,
          vpcSubnets: {
            subnetType: SubnetType.PRIVATE_WITH_EGRESS,
          },
          runtime: Runtime.NODEJS_22_X,
          layers: [layer_lambda],
          code: Code.fromAsset(lambda_update_group_hierarchy_path),
          handler: "index.handler",
          timeout: Duration.seconds(50),
          logGroup: lambda_update_group_membership_log,
          architecture: Architecture.ARM_64,
        ...
        lambda_update_group_hierarchy.node.addDependency(lambda_update_group_hierarchy_log)
 
        lambda_update_group_hierarchy.addToRolePolicy(
          new PolicyStatement({
        ...
        // IOT RULE FOR GROUP HIERARCHY EVENTS
        const iot_rule_group_hierarchy = new CfnTopicRule(this, "GarnetIoTGroupHierarchyRule", {
          ruleName: `garnet_iot_thing_group_hierarchy_rule`,
          topicRulePayload: {
            awsIotSqlVersion: "2016-03-23",
            ruleDisabled: false,
            sql: `SELECT *, topic(7) as thingName, topic(5) as thingGroup from '$aws/events/thingGroupHierarchy/#'`,
            actions: [
              {
                lambda: {
                  functionArn: lambda_update_group_hierarchy.functionArn
                }
              }
            ]
          }
        ...
        // GRANT IOT RULE PERMISSION TO INVOKE LAMBDA
        new CfnPermission(this, 'GarnetIoTGroupHierarchyLambdaPermission', {
          principal: `iot.amazonaws.com`,
          action: 'lambda:InvokeFunction',
          functionName: lambda_update_group_hierarchy.functionName,
          sourceArn: `${iot_rule_group_hierarchy.attrArn}`
        })

        /*
         *  END THING GROUP HIERARCHY 
         */
    }
}
