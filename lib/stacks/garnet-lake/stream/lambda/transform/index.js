...
const GARNET_LAKE_WIRE_PREFIX = Buffer.from([0x47, 0x4c, 0x42])
const GARNET_LAKE_WIRE_VERSION = 1
...
const decodeEntityEvent = (data) => {
  const wire = Buffer.from(data, "base64")
  if (
    wire.length >= GARNET_LAKE_WIRE_PREFIX.length &&
    wire.subarray(0, GARNET_LAKE_WIRE_PREFIX.length)
      .equals(GARNET_LAKE_WIRE_PREFIX)
  ) {
    const version = wire[GARNET_LAKE_WIRE_PREFIX.length]
    if (version !== GARNET_LAKE_WIRE_VERSION) {
      throw new Error(`unsupported Garnet Lake wire version ${version}`)
    }
    const compact = JSON.parse(
      brotliDecompressSync(
        wire.subarray(GARNET_LAKE_WIRE_PREFIX.length + 1)
      ).toString("utf8")
    )
    if (!Array.isArray(compact) || compact.length !== 8) {
      throw new Error("compact Entity event must contain 8 fields")
    }
    return {
      schemaVersion: 1,
      eventId: compact[0],
      tenant: compact[1],
      entityId: compact[2],
      committedAt: compact[3],
      aggregateVersion: compact[4],
      controlCursor: compact[5],
      operation: compact[6],
      payload: compact[7]
    }
  }
...
const transformRecord = (
  ...
  try {
    const event = decodeEntityEvent(record.data)
